...
#declare -r VERSION=1.1d            ### Remove -T -100 from the kiwirecorder command line to completely disable squelch.  Fix bug in add_remove_jobs_in_running_file(), missing 'grep -w'
                                    ###     Add WSPRD_CMD_FLAGS (default = "-d") which can be modified by redeclaring it in kiwiwspr.conf
...
function spawn_recording() {
    ...
    local capture_client_name=${KIWIRECORDER_CLIENT_NAME:-kiwiwspr_v${VERSION}}
    python -u ${CAPTURE_COMMAND} \
        --user=${capture_client_name}  --agc-gain=60 --quiet --no_compression \
        --server-host=${kiwi_ip/:*} --server-port=${kiwi_ip#*:} \
        --freq=${kiwi_rx_freq_khz} --modulation=usb  --lp-cutoff=1250 --hp-cutoff=1750 \
        --password=${my_kiwi_password} --dt-sec=120  >> capture.log 2>&1 &
    echo $! > capture.pid
    [[ $verbosity -ge 2 ]] && set +x
...
if [[ "${OSTYPE}" == "darwin17" ]]; then
    ...
    declare -r SECOND_WSPRD_CMD=/usr/bin/wsprd.v1
else
    ### TODO:  
    echo "ERROR: We are running on a OS '${OSTYPE}' which is not yet supported"
    exit 1
...
if ${WSPRD_CMD} | grep -q '\-o' ; then
    declare WSPRD_CMD_FLAGS="-d -C 5000 -o 4"
    [[ ${verbosity} -ge 1 ]] && echo "$(date): INFO: ${WSPRD_CMD} is version 2, so set command line flags to '${WSPRD_CMD_FLAGS}'"
else
    declare WSPRD_CMD_FLAGS="-d -C 10000"        ### Default to do deep decode.  Can be overwritten by re-declaring in kiwiwspr.conf.  It takes about 30 CPU seconds to run on a Pi B3 core
    [[ ${verbosity} -ge 1 ]] && echo "$(date): INFO: ${WSPRD_CMD} is version 1, so set command line flags to '${WSPRD_CMD_FLAGS}'"
fi

declare -r WSPRD_DECODES_FILE=wsprd.txt               ### wsprd stdout goes into this file, but we use ALL_WSPR.TXT
declare -r WSPRNET_UPLOAD_FILE=wsprd_upload.txt       ### The output of wsprd is reworked by awk into this file which is uploaded by curl to wsprnet.org
declare -r WSPRNET_UPLOAD_LOG=wsprd_upload.log        ### Log of our curl uploads

declare    SECOND_DECODE_PASS_ENABLED="no"            ### change to "yes" if you want to first run wsprd in quick mode and then compare its results against the deep mode.  That is no longer interesting to me
declare    SECOND_WSPRD_CMD_FLAGS="-d"                ### Default to do deep decode.  Can be overwritten by re-declaring in kiwiwspr.conf.  It takes about 30 CPU seconds to run on a Pi B3 core
declare -r SECOND_DECODER_DECODES_FILE=new_wsprd.txt
declare -r SECOND_DECODE_UPLOAD_FILE=new_wsprd_upload.txt
declare -r SECOND_DECODE_UPLOAD_LOG=new_wsprd_upload.log

declare -r WAV_FILE_POLL_SECONDS=2            ### How often to poll for the 2 minute record file to be filled
...
function wspr_decode_daemon() 
{
    ...
    while true; do
        ...
        for wav_file_name in *.wav; do
            [[ ${verbosity} -ge 1 ]] && echo "$(date): processing wav file '${wav_file_name}'. Waiting for file size to stabilize.."
            ...
            while [[ ${new_wav_file_size} -ne ${old_wav_file_size} ]]; do
                old_wav_file_size=${new_wav_file_size}
                sleep ${WAV_FILE_POLL_SECONDS}
                new_wav_file_size=$( ${GET_FILE_SIZE_CMD} ${wav_file_name} )
            done
            [[ ${verbosity} -ge 1 ]] && echo "$(date): wav file '${wav_file_name}' stabilized at size ${new_wav_file_size}."
            if  [[ ${new_wav_file_size} -lt ${WSPRD_WAV_FILE_MIN_VALID_SIZE} ]]; then
                [[ ${verbosity} -ge 0 ]] && echo "$(date): wav file '${wav_file_name}' size ${new_wav_file_size} is too small to be processed by wsprd.  Delete this file and go to next wav file."
                rm -f ${wav_file_name}
                continue
            fi
            ...
            rm -f ALL_WSPR.TXT
            touch ALL_WSPR.TXT
            nice ${WSPRD_CMD} ${WSPRD_CMD_FLAGS} -f ${wspr_decode_capture_freq_mhz} ${wav_file_name} > ${WSPRD_DECODES_FILE}
            if [[ -s ALL_WSPR.TXT ]]; then
               ### wsprd gives lines in this format, but since ALL_WSPR.TXT contains the date and time I use it as the source:
                ...
                ###   1     2    3          4  5   6    7    8      9
                ### We rework ALL_WSPR.TXT with awk to output lines to wsprnet.org in this format:
                ...
                if [[ -s ${WSPRNET_UPLOAD_FILE} ]]; then
                    ...
                    if ! curl -F allmept=@${WSPRNET_UPLOAD_FILE} -F call=${my_call_sign} -F grid=${my_grid} ${SPOTS_URL} > curl.log 2>&1; then
	   ...
	   if [[ ${SECOND_DECODE_PASS_ENABLED} == "yes" ]] && [[ -x ${SECOND_WSPRD_CMD} ]]; then
		mkdir -p new_decodes.d
                cd new_decodes.d
                rm -f ALL_WSPR.TXT
                touch ALL_WSPR.TXT
                nice ${SECOND_WSPRD_CMD} ${SECOND_WSPRD_CMD_FLAGS} -f ${wspr_decode_capture_freq_mhz} ../${wav_file_name} > ${SECOND_DECODER_DECODES_FILE}
                if [[ -s ALL_WSPR.TXT ]]; then
                    awk -v date=${wspr_decode_capture_date} -v time=${wspr_decode_capture_time} -v snr_adj=${KIWI_SNR_ADJUST} \
                        '/_usb/{printf "%s %s 1 % 3.0f % 5.1f %10s %10s %6s %4s %3s\n", date, time, ($4 + snr_adj), $5, $6, $7, $8, $9, $10}' \
                        ALL_WSPR.TXT  > ${SECOND_DECODE_UPLOAD_FILE}
                    if [[ -s ${SECOND_DECODE_UPLOAD_FILE} ]]; then
                        cat ${SECOND_DECODE_UPLOAD_FILE} >> ${SECOND_DECODE_UPLOAD_LOG}
		        local my_new_call_sign="${my_call_sign}1"  ## <=== NOTE the '1' added to the call sign for logging these decodes
                        if ! curl -F allmept=@${SECOND_DECODE_UPLOAD_FILE} -F call=${my_new_call_sign} -F grid=${my_grid} ${SPOTS_URL} > curl.log 2>&1; then
                            echo "$(date): ERROR: new wsprd curl => $?:"
                            cat curl.log
                        fi
                    fi
                    if ! diff ${SECOND_DECODE_UPLOAD_FILE} ../${WSPRNET_UPLOAD_FILE} > diff.log ; then
                        local new_spot_count=$(cat ${SECOND_DECODE_UPLOAD_FILE} | wc -l)
                        local old_spot_count=$(cat ../${WSPRNET_UPLOAD_FILE} | wc -l )
                        echo "$(date): ==== There were ${new_spot_count} decodes using wsprd.v2 and ${old_spot_count} decodes using wsprd.v1.9.1 ======"
                        cat diff.log
                    fi
                fi
                cd ..
            fi
            [[ ${verbosity} -ge 1 ]] && echo "$(date): done processing wav file '${wav_file_name}'"
            if [[ -f ${wav_file_name} ]]; then
                rm ${wav_file_name}
            else
                echo "ERROR: wav filee '${wav_file_name}' did not exist at the end of the processing loop"
            fi
        done
        sleep ${WAV_FILE_POLL_SECONDS}
    done
...
while getopts :dhj:vVw: opt ; do
    case $opt in
        ...
        V)
            ...
            ;;
        d)
            SECOND_DECODE_PASS_ENABLED="yes"
            ;;
        \?)
