...
spawn_recording() {
    ...
    [[ $verbosity -ge 2 ]] && \
        echo "INFO: Running  \"python -u ${CAPTURE_COMMAND} -q --ncomp -s ${kiwi_ip/:*} -p ${kiwi_ip#*:} -f ${kiwi_rx_freq_khz} -m usb  -L 1200 -H 1700  --pw=${my_kiwi_password} -T -101 --dt-sec 120  >> capture.log 2>&1 &\""
...
function add_remove_jobs_in_running_file() {
    ...
    case $action in
        a)
            if grep ${job} ${RUNNING_JOBS_FILE} > /dev/null; then
